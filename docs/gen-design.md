...
- **G4 LoRA 棚** ✅ 2026-09-05 実装: src/lora.rs(store/lora/<stem>.safetensors + json)、URL 取り込み(Hugging Face リポジトリ/直リンク、Civitai モデル/バージョン、直リンク。親モデルを baseModel/名前から当てて内蔵モデルに載らない物は拒否)、.safetensors のドラッグ&ドロップ、試し描き 2 題(内蔵エンジン、previews/<stem>_N.jpg)、カード(作例・親モデルバッジ・トリガー語・出典・使用中フォルダ)、「これで作る」で LoRA を着せた生成フォルダ、生成パネルの LoRA 行(親モデルが合う物だけ候補・強さ)。sd-cli は `<lora:stem:scale>`、sd-server は `lora:[{path,multiplier}]`。Civitai の DL は設定の API キー(keys.civitai)。**親モデル違いは選べない**(2026-09-05 指示): 「足す」候補は選択中モデルに合う物だけ、モデルを切り替えたら合わない LoRA は自動で外して通知、サーバ側も生成時に弾く(親モデル不明は klein 用扱い)。棚は親モデルごとのタブ(すべて/klein/Z-Image/Qwen/非対応、件数つき)で「何を持っているか」が見える。
