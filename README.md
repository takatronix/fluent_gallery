...
| **マスク** | 内蔵SAM2(sam2-hiera-tiny・Apache-2.0・155MB)。ライトボックスで「マスク」をONにして、クリック=そこを切る / ドラッグ=範囲で切る / 右クリック=消す。切り抜きPNG書き出し |
...
FG_URL=http://127.0.0.1:18890 npm --prefix tests run test:studio-float  # 写真サイズ維持・フロート移動・最小化・再編集を検証
...
画像を開いて「編集」（E）を押すと、露出・コントラスト・彩度・色温度・自動補正・回転・反転・クロップは従来どおり写真の下で操作できる。「ぼかし」の右にある操作グループの「言語・フィルター」で、言語指示・個別フィルター・🎲・リセットだけをフロート表示する。フィルターパネルの開閉や移動は写真と基本編集欄の配置を変えない。見出しをドラッグして移動でき、最小化や×で隠しても編集中の設定は保持する。fluent_sceneの実際のノードとパラメーターUIをフロート内に配置し、結果をギャラリーの写真へ表示する。フィルター編集中の写真調整は下書きで、既存の「適用」ボタンが両方を原寸PNGへ保存する。適用しても画像のIDと一覧の枚数は変わらない。加工済みPNGは編集履歴が参照する非公開ファイルとして保存し、元画像は不変のまま保持する。閲覧時は保存済み画像を読み、再編集時は元画像とレシピから復元する。「一手戻す」は下書きの写真調整・フィルターと、適用済みの編集履歴に対応する。「フィルターをリセット」は写真調整を残し、「原本に戻す」は同じ画像の編集を戻す。保存構造は [docs/studio-storage.md](docs/studio-storage.md)。上流の更新は `python3 tools/sync_scene_editor.py ../fluent_scene` で取り込む。
