...
pub fn defaults() -> Value {
    let mut v = json!({
        ...
        "gen": {"base": "", "port": 8092, "size": "1024x1024", "model": "flux2-klein-4b", "preview": true, "offload": "auto"}, // base=別マシンの sd-server(空=内蔵)。preview=途中経過(sd-cli)。offload=auto|on|off(--offload-to-cpu)
